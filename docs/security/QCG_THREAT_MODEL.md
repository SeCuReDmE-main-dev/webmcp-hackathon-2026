# WebMCP-QCG v2 threat model

Status: release baseline
Date: 2026-08-29
Scope: browser-local Q# preflight, bounded Bell simulation and evidence export
...
1. **Human file boundary** — the human chooses a local `.qs` file. The source remains in session memory and enters the local Q# Worker only.
2. **WebMCP boundary** — the agent receives identifiers, bounded manifests, recommendations and receipts. Raw Q#, local paths and credentials stay outside tool inputs and outputs.
3. **Worker boundary** — `qsharp-lang@1.31.0` compiles and runs the published Bell contract in a dedicated Worker with shot, qubit and timeout limits.
4. **Persistence boundary** — IndexedDB stores evidence receipts only. Source code and consent tokens are not durable records.
5. **External boundary** — no provider API, QPU job, credential, payment or remote quantum execution exists in the MVP.
...
| Raw Q# or private paths leak through agent responses | Compact output schemas, bounded errors and export tests; source stays outside WebMCP and IndexedDB | Browser extensions and the host OS remain outside QCG's trust claim |
...
This threat model does not claim provider authentication, cloud budget enforcement, QPU isolation, scientific correctness for arbitrary Q#, regulatory compliance, penetration-test coverage or protection against a compromised browser/operating system.

## Release evidence

See [the live-origin acceptance receipt](../../evidence/qa/LIVE_ORIGIN_ACCEPTANCE_RECEIPT_2026-08-29.md) and [the native Q# receipt](../../evidence/qa/NATIVE_WEBMCP_QSHARP_RECEIPT_2026-08-29.md).
