...
assert(contentBridge.includes("type: 'qcg-console-open-side-panel', request_id: requestId") && !contentBridge.includes('tab_id: data.'), 'content bridge must not forward a page-supplied tab id')
...
const openCompanionBody = background.slice(background.indexOf('async function openCompanion(tabId)'), background.indexOf('function deliverSnapshot'))
