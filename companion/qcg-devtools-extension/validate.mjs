...
assert(contentBridge.includes("qcg-console-extension-control.v1") && contentBridge.includes("data.type === 'open_companion'") && contentBridge.includes("type: 'open_companion_result'") && contentBridge.includes('event.source !== window || event.origin !== location.origin'), 'page open-companion handshake must be same-window, same-origin, explicit, and paired')
assert(contentBridge.includes("type: 'qcg-console-open-side-panel', request_id: data.request_id") && !contentBridge.includes('tab_id: data.'), 'content bridge must not forward a page-supplied tab id')
...
for (const label of ['Inspector', 'Console', 'WebMCP', 'Decisions', 'Sources', 'Receipts', 'Activity']) assert(read('panel.html').includes(label), `shared console navigation missing ${label}`)
