...
const OPEN_PANEL_STATE_KEY = 'qcg-open-side-panel-tabs-v1'
const panelStateReady = restoreOpenPanelState()
...
chrome.action.onClicked.addListener((tab) => { if (typeof tab.id === 'number') void openCompanion(tab.id, 'open') })
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'qcg-console-open-side-panel' && validRequestId(message.request_id) && validCompanionAction(message.action)) {
    const tabId = sender.tab?.id
    if (!Number.isInteger(tabId)) { sendResponse({ request_id: message.request_id, opened: 'none' }); return }
    void openCompanion(tabId, message.action).then((result) => sendResponse({ request_id: message.request_id, ...result }))
    return true
  }
  if (message.type === 'qcg-console-open-side-panel-for-extension' && Number.isInteger(message.tab_id)) {
    void openCompanion(message.tab_id, validCompanionAction(message.action) ? message.action : 'open').then((result) => sendResponse(result))
    return true
  }
...
function attachContent(port) {
  ...
  void panelStateReady.then(() => notifyContentCompanionState(tabId, openSidePanelTabs.has(tabId)))
...
function attachSidePanel(port) {
  ...
  port.onMessage.addListener((message) => {
    ...
    if (message.type === 'qcg-console-attach.v1' && Number.isInteger(message.tab_id)) {
      ...
      openSidePanelTabs.add(tabId); void persistOpenPanelState(); notifyContentCompanionState(tabId, true)
...
async function openCompanion(tabId, action) {
  if (!contentPorts.has(tabId)) {
    void disableCompanion(tabId)
    return { opened: 'none', reason: 'unsupported_tab' }
  }
  if (action === 'close' && chrome.sidePanel?.close) {
    try {
...
async function restoreOpenPanelState() {
  try {
    const stored = await chrome.storage?.session?.get(OPEN_PANEL_STATE_KEY)
    const tabIds = stored?.[OPEN_PANEL_STATE_KEY]
    if (Array.isArray(tabIds)) for (const tabId of tabIds) if (Number.isInteger(tabId)) openSidePanelTabs.add(tabId)
  } catch {}
}
async function persistOpenPanelState() {
  try { await chrome.storage?.session?.set({ [OPEN_PANEL_STATE_KEY]: [...openSidePanelTabs] }) } catch {}
}
