...
function safeSnapshot() {
  ...
  return {
    ...
    tools: list(value.tools, ['name', 'group', 'status', 'last_invocation'], 12)
  }
}
...
function list(value, keys, max = 50) { return Array.isArray(value) ? value.slice(0, max).map((item) => pick(item, keys)).filter(Boolean) : [] }
