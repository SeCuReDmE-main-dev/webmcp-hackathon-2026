...
describe('third-party DevTools discovery lifecycle', () => {
  it('registers only the four narrow collaboration tools', async () => {
    ...
    const bridge = {
      ...
      queueHumanReviewAcknowledgement: vi.fn()
    }
    ...
    expect(group.tools.map((tool: { name: string }) => tool.name)).toEqual(['read_debug_context', 'post_debug_observation', 'request_human_review', 'export_debug_handoff'])
    const read = group.tools[0]
    expect(await read.execute({})).not.toHaveProperty('consent')
    const post = group.tools[1]
    await post.execute({ schema_version: 'qcg-debug-message.v1', session_id: sessionId, actor: 'codex', role: 'reviewer', kind: 'observation', summary: 'The visible state remains bounded.', evidence_refs: [], confidence: 'high', status: 'open', identity_assurance: 'declared' })
    expect((await ledger.messages(sessionId))).toHaveLength(1)
    await expect(post.execute({ schema_version: 'qcg-debug-message.v1', session_id: sessionId, actor: 'codex', role: 'reviewer', kind: 'observation', summary: 'Caller-owned metadata must be rejected.', evidence_refs: [], confidence: 'high', status: 'open', identity_assurance: 'declared', event_id: crypto.randomUUID(), issued_at: '2099-01-01T00:00:00.000Z' })).rejects.toThrow()
    await expect(post.execute({ schema_version: 'qcg-debug-message.v1', session_id: crypto.randomUUID(), actor: 'codex', role: 'reviewer', kind: 'observation', summary: 'A cross-session write attempt.', evidence_refs: [], confidence: 'high', status: 'open', identity_assurance: 'declared' })).rejects.toThrow('active QCG page session')
  ...
  it('records a declared Codex to Gemini to human exchange without changing quantum authority', async () => {
    ...
    const panel = async () => ({ schema_version: 'qcg-devtools-context.v1' as const, ...authority, storage_mode: 'memory' as const, session_id: sessionId, messages: await ledger.messages(sessionId), participants: [], human_review_requests: [] })
    const bridge = {
      ...
      getCachedPanelSnapshot: () => ({ schema_version: 'qcg-devtools-context.v1' as const, ...authority, storage_mode: 'memory' as const, session_id: sessionId, messages: [], participants: [], human_review_requests: [] }),
      appendHumanMessage: vi.fn(),
    ...
    const base = { schema_version: 'qcg-debug-message.v1', session_id: sessionId, evidence_refs: ['receipt:seasonal-runtime'], confidence: 'high', status: 'open', identity_assurance: 'declared' }
    await tools.post_debug_observation.execute({ ...base, actor: 'codex', role: 'code-reviewer', kind: 'observation', summary: 'The authority boundary remains intact.' })
    await tools.post_debug_observation.execute({ ...base, actor: 'gemini', role: 'browser-reviewer', kind: 'observation', summary: 'Counter-analysis confirms that no consent control crosses the bridge.' })
