...
function requestCompanion(requestId) {
  void chrome.runtime.sendMessage({ type: 'qcg-console-open-side-panel', request_id: requestId }).then(
    (result) => window.postMessage({ channel: CONTROL_CHANNEL, type: 'open_companion_result', request_id: requestId, status: openStatus(result?.opened), reason: openReason(result?.reason) }, location.origin),
...
document.addEventListener('click', (event) => {
  ...
  if (validRequestId(requestId)) requestCompanion(requestId)
}, true)
...
window.addEventListener('message', (event) => {
  ...
  if (data.type === 'snapshot') {
    ...
    if (snapshot) port.postMessage({ type: 'qcg-console-snapshot.v1', snapshot })
  }
  if (data.type === 'command_result' && validRequestId(data.request_id)) {
    const result = sanitizeResult(data.result)
    if (result) port.postMessage({ type: 'qcg-console-command-result.v1', request_id: data.request_id, result })
  }
})
...
function openReason(value) { return ['opened', 'closed', 'side_panel_declined', 'browser_declined'].includes(value) ? value : 'browser_declined' }
